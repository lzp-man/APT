...
class ModelArguments:
    ...
    lora_rank: int = field(
        default=64,
        metadata={"help": "The rank of LoRa, determining the dimensionality of the low-rank factorization."}
    )
    lora_alpha: float = field(
        default=16,
        metadata={"help": "The alpha parameter of LoRa, affecting the scaling of the low-rank transformation."}
    )
    lora_dropout: float = field(
        default=0.1,
        metadata={"help": "The dropout rate for LoRa modules, used to prevent overfitting."}
    )
...
class DataTrainingArguments:
    ...
    eval_file: Optional[str] = field(default=None, metadata={"help": "The input evaluate data file (a json/jsonl file)."})
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        model_args, data_args, training_args,generating_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args,generating_args = parser.parse_args_into_dataclasses()
    ...
    if model_args.model_name_or_path:
        if model_args.use_lora:
            ...
            if model_args.lora_path:
                ...
                logger.info("no lora_path and just config myself!")

...
from dpo_utils_capt import dpo_loss,dpo_loss_w_reg, concatenated_forward,concatenated_forward_orpo,odds_ratio_loss, DataCollatorForSeq2SeqDPO
...
def parse_args():
    ...
    parser.add_argument(
        "--use_orpo_loss",
        action="store_true",
        help="If passed, will use orpo loss to replace dpo loss.",
    )
    parser.add_argument(
        "--use_reg_dpo_loss",
        action="store_true",
        help="If passed, will use the dpo_loss with regular term.",
    )
    parser.add_argument(
        '--use_sft_reg',
        action='store_true',
        help='Use sft loss as dpo reg',
    )
    parser.add_argument(
        "--lora_rank",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help='alpha parameter for DPO regular term. Default is 0.5.',
    )
    parser.add_argument(
        '--beta',
        type=float,
                ...
                if not args.use_orpo_loss:
                    ...
                    if args.use_reg_dpo_loss:
                        if args.use_sft_reg:
                            ...
                            losses, chosen_rewards, rejected_rewards = dpo_loss(              # return loss, chosen_rewards, rejected_rewards
                    ...
                    policy_chosen_logps, policy_rejected_logps,policy_nll_loss = concatenated_forward_orpo(model,batch)
                    losses, chosen_rewards, rejected_rewards = odds_ratio_loss(
                        policy_chosen_logps, policy_rejected_logps,beta=args.beta
                    )
                    #print(f"policy_nll_loss:{policy_nll_loss}")
                    #print(f"odd_loss:{losses.mean()}")
